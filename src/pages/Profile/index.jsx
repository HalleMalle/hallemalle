...
export default function Profile() {
  ...
  const handleDisplayNameSubmit = async (event) => {
    ...
    try {
      ...
      setIsEditingName(false);
      setSuccessMessage("노출명이 저장되었습니다.");
    } catch (error) {
      setErrorMessage(getProfileErrorMessage(error));
    } finally {
      setIsSaving(false);
    ...
    return (
      <main className="profile-page">
        <div className="container">
          <div className="profile-status-panel">프로필 정보를 불러오는 중입니다.</div>
        </div>
      </main>
    );
  }
  ...
  if (!profile) {
    return (
      <main className="profile-page">
        <div className="container">
          <div className="profile-status-panel profile-status-panel-error" role="alert">
  ...
  const avatarLabel = profile.display_name || profile.github_login || DEFAULT_AVATAR_LABEL;
  const avatarImage = profile.photo_url || profile.photoURL || "";
  const tierLabel = `${profile.tier || "bronze"} ${profile.tier_detail || 1}`;
  const collaborationScore = Number(profile.collaboration_score ?? 10).toFixed(1);
  ...
  return (
    <main className="profile-page">
      <div className="container">
        <section className="profile-section profile-section-main" aria-labelledby="profile-title">
          <div className="profile-main-header">
            ...
            {!isEditingName && (
              <button type="button" className="profile-secondary-button" onClick={handleEditName}>
                수정
              </button>
            )}
          </div>
          ...
          {isEditingName ? (
            <form className="profile-name-form" onSubmit={handleDisplayNameSubmit}>
              <label htmlFor="displayName">노출명</label>
              ...
              <p className="profile-help-text">2~20자, 공백 없이 입력해주세요.</p>
            </form>
          ) : (
        ...
        <section className="profile-section" aria-labelledby="profile-github-title">
          <div className="profile-section-header">
            <div>
          ...
          <div className="profile-metrics">
            ...
            <ProfileMetric label="저장소" value={profile.github_repositories || 0} />
            <ProfileMetric label="기여도" value={profile.github_contributions || 0} />
            <ProfileMetric label="팔로워" value={profile.github_followers || 0} />
            <ProfileMetric label="스타" value={profile.github_stars || 0} />
          </div>
        </section>

        <section className="profile-section" aria-labelledby="profile-links-title">
          <div className="profile-section-header">
            <div>
